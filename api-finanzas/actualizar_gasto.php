...
if ($datos) {
    ...
    $stmt = $conexion->prepare("UPDATE transacciones SET usuario_id = ?, categoria_id = ?, importe = ?, fecha = ?, descripcion = ? WHERE id = ?");
    
    // "iids s" significa: entero, entero, decimal(double), string, string
    $stmt->bind_param("iidssi", $user, $cat, $imp, $fech, $desc, $id);
